...
module.exports.handler = async (event) => {
  ...
  try {
    ...
    response = await client.query(
      "select distinct shipment_info.file_nbr ,shipment_info.house_bill_nbr ,shipment_info.bill_to_nbr ,shipment_info.origin_port_iata ,shipment_info.destination_port_iata , shipment_info.shipper_name ,shipment_info.shipper_addr_1 ,shipment_info.shipper_addr_2 ,shipment_info.shipper_city ,shipment_info.shipper_st , shipment_info.shipper_cntry ,shipment_info.shipper_zip , shipment_info.consignee_name ,shipment_info.consignee_addr_1 ,shipment_info.consignee_addr_2 , shipment_info.consignee_city ,shipment_info.consignee_st , shipment_info.consignee_cntry ,shipment_info.consignee_zip , shipment_milestone.order_status ,shipment_milestone.order_status_desc ,shipment_milestone.region_code_basis ,shipment_Ref.ref_nbr from shipment_info left outer join shipment_milestone on shipment_info.source_system = shipment_milestone.source_system and shipment_info.file_nbr = shipment_milestone.file_nbr and is_custompublic = 'Y' and shipment_milestone.order_status in ('NEW','APP','PUP','AAD','VER', 'APD','TTC','OFD','DEL','WEB') left outer join (select source_system ,file_nbr ,ref_nbr from (select distinct source_system ,file_nbr ,pk_Ref_nbr,ref_nbr, rank() over (partition by file_nbr order by ref_typeid,pk_Ref_nbr)rk from shipment_ref where ref_typeid in  ('PO','REF') and customer_type = 'B')Z where z.rk = 1) shipment_Ref on shipment_info.source_system = shipment_Ref.source_system and shipment_info.file_nbr = shipment_Ref.file_nbr WHERE shipment_info.bill_to_nbr = '11912' and shipment_milestone.event_date > convert_timezone('CDT', sysdate) - interval '7 day'"
    );
      ...
      if (inputRecord.length) {
        try {
          ...
          for (let x in recordInsert) {
            console.info("Insert New Record ==> 258 : ", JSON.stringify(recordInsert));
        ...
        if (matchedRecord.length) {
          console.info("Info ==> 266 : Matched Records Length : ", matchedRecord.length);
          console.info("Info ==> 267 : Matched Records : ", JSON.stringify(matchedRecord));
