                    ...
                    if(fileExists("P44BatchLoad/Dockerfile")){
                        dir("./P44BatchLoad"){
                            withAWS(credentials: 'omni-aws-creds'){
                                sh """ 
                                aws ecr get-login-password --region us-east-1 | docker login --username AWS https://332281781429.dkr.ecr.us-east-1.amazonaws.com -p $(aws ecr get-login-password --region us-east-1)
